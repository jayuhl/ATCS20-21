...
function runGame(canvas){
    ...
    enemies = [];
    // enemies.push(new sprite(30, 30, 50, 50, 0, 1, 1, "yellow"));
    // enemies.push(new sprite(30, 80, 50, 50, 0, 1, 1, "orange"));
...
//use the ones below for some good responses... delay=50 watch dark and light blue at aourd 12-16 on timer, nice collision at 16... 
...
enemies.push(new sprite(0, -600, 600, 600, 0, 0, 0, "black"));
enemies.push(new sprite(-500, 0, 500, 500, 0, 0, 0, "black"));
enemies.push(new sprite(600, 0, 500, 500, 0, 0, 0, "black"));
enemies.push(new sprite(0, 500, 600, 600, 0, 0, 0, "black"));
...
class sprite{
    ...
    //checks if NEXT location is already occupied
    collidingWith(other){
        if(Math.abs((this.x + this.dx + this.w/2) - (other.x + other.w/2 + other.dx)) <= this.w/2 + other.w/2
                  && Math.abs((this.y + this.dy + this.h/2) - (other.y + other.h/2 + other.dy)) <= this.h/2 + other.h/2){
                    //   console.log("CRASH " + this.color + " , " + other.color);
    ...
    resolveCollision(other){
        ...
        // console.log(angle);
        if(Math.abs(angle) < 45){//Collision is from the left or right
        ...
        else{ //Collision is from the top or bottom
